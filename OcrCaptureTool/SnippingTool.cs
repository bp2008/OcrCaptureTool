...
namespace OcrCaptureTool
{
	...
	{
		static Font font = new Font(FontFamily.GenericMonospace, 12);
		public static void DrawOverlayAcrossAllScreens(MainForm frmMain)
		{
			...
			hud.Paint += (object sender, PaintEventArgs e) =>
			{
				...
				if (isMouseDown)
				{
					...
					label = w + " x " + h;
					if (w < 10 || h < 10)
						label += Environment.NewLine + "(too small)";
				...
				{
					label = "Click and drag to select a region." + Environment.NewLine + "Right click to cancel selection.";
				...
				if (isMouseDown && e.Button == MouseButtons.Left)
				{
					...
					if (finalSnippingSelection.Width >= 10 && finalSnippingSelection.Height >= 10)
					{
						try
						{
							...
							bg.Hide();
							bg.Opacity = 0;
							frmMain.AcceptUserSelectedRectangle(finalSnippingSelection);
						}
						catch (Exception ex)
						{
